   ...
   */
   ...
   function removeBox() {
      $(btnRemoveItem).click(function (e) {
         ...
         }else if (window.location.pathname.split("/")[2] == 'invoices' && window.location.pathname.split("/")[3] == 'edit-journal'){
            let data = {
                id: $(this).attr("data-id")
             };
             $.post(baseUrl + $("#prefix").val() + '/invoices/item/journal_item_destory', data,
                function (data, textStatus, jqXHR) {
                },
                "json"
             );
...
$('.request-set-paid-button').on('click',function (){
   let action = $(this).data('url');
   console.log(action);
   $('#refusedPaidInvoiceForm').attr('action',action);
   $('#refusedPaidInvoice').modal('show');
});

$('.request-certificate-input-button').on('click',function (){
   let action = $(this).data('url');
   $('#refusedCertificateForm').attr('action',action);
   $('#refusedCertificate').modal('show');
});
